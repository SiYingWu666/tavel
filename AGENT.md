...
- Mobile goal: keep the app usable on phones and desktops. It is configured as a PWA with manifest/service worker and should support adding to the home screen.
...
- For phone testing on the same Wi-Fi, use `npm run dev:host` or `npm run preview:host`.
